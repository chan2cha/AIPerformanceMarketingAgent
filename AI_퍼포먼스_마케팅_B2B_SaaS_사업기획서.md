...
브랜드 / 제품 / 업종 / 타깃 시장 / 경쟁사
        ↓
[수집 대상 자동 설계]
플랫폼 / 경쟁 브랜드 / 업종 키워드 / 국가 / 언어
        ↓
[시장·경쟁사 콘텐츠 자동 탐색 및 수집]
Meta Ad Library / TikTok Creative Center·광고 라이브러리 / 기타 허용된 데이터 소스
        ↓
[콘텐츠 정규화]
중복 제거 / 광고주 식별 / 원문·이미지·영상·랜딩페이지 연결
        ↓
[AI 전처리]
본문 추출 / OCR / 음성·자막 추출 / 언어 감지
        ↓
[광고 패턴 구조화]
Hook / Pain Point / Offer / CTA / Format / Product / Audience
        ↓
[시장 패턴·변화 감지]
경쟁사별 반복 패턴 / 업종 트렌드 / 신규 소재 / 장기 집행 신호
        ↓
[자사 과거 성과와 비교]
        ↓
[다음 광고 가설 생성]
...
이 제품에서 사용자가 반복적으로 해야 하는 일은 광고 소재를 직접 찾아 등록하는 것이 아니다.

사용자는 최초에 다음 정보만 설정한다.

- 자사 브랜드, 제품, 업종, 타깃 고객
- 분석할 국가와 언어
- 경쟁 브랜드와 각 브랜드의 Meta 페이지, TikTok 계정, 웹사이트
- 추가로 관찰할 업종 키워드

이후 시스템이 Script 생성 전 단계인 **탐색 → 수집 → 정리 → 분석 → 비교 → 가설 생성 → Brief 생성**을 주기적으로 자동 실행한다. 수동 URL·소재 등록은 공식 데이터 소스에서 찾지 못한 콘텐츠를 보완하는 예외 기능으로 둔다.

### 4.3.1 Script 생성 전 자동화 파이프라인

| 단계 | 시스템 동작 | 주요 산출물 |
|---|---|---|
| 1. Source Planning | 브랜드·업종·국가를 바탕으로 플랫폼별 검색 대상과 키워드 구성 | 수집 계획 |
| 2. Discovery | 경쟁사 계정과 업종 키워드로 신규 광고·콘텐츠 탐색 | 후보 콘텐츠 |
| 3. Collection | 허용된 API·데이터 소스를 통해 원문과 메타데이터 수집 | Raw Creative |
| 4. Normalization | 플랫폼별 데이터를 공통 형식으로 변환하고 중복 제거 | Canonical Creative |
| 5. Extraction | 광고 문구, OCR, 영상 자막·음성, 랜딩페이지 핵심 내용 추출 | 분석 가능한 텍스트·미디어 정보 |
| 6. Classification | Hook, Offer, CTA, Format, Pain Point, 제품, 타깃 등을 구조화 | Creative Analysis |
| 7. Pattern Mining | 경쟁사·업종·기간별 반복 패턴과 변화를 집계 | Market Pattern |
| 8. Performance Match | 자사 광고 성과가 있으면 유사 패턴의 성과와 비교 | Performance Evidence |
| 9. Hypothesis | 근거와 신뢰도를 포함한 다음 광고 실험 제안 | Creative Hypothesis |
| 10. Brief | 제품 USP와 브랜드톤을 적용해 제작 요건으로 변환 | Creative Brief |
| 11. Script | 승인된 Brief를 채널·포맷별 대본과 카피로 변환 | Script / Copy |

Script는 단순 프롬프트로 바로 생성하지 않는다. 각 Script는 최소한 다음 근거를 추적할 수 있어야 한다.

- 어떤 시장·경쟁사 패턴을 참고했는지
- 해당 패턴을 뒷받침하는 광고 소재가 무엇인지
- 자사 성과 데이터가 반영되었는지
- 왜 이 가설을 추천했는지
- 근거의 최신성과 신뢰도가 어느 정도인지

### 4.3.2 수집 원칙

- 공식 API, 공식 공개 데이터, 계약된 데이터 제공업체를 우선한다.
- 플랫폼별 접근 범위와 국가 제한을 명시하고, 수집 불가를 정상 상태로 처리한다.
- 화면 스크래핑에 제품의 핵심 가치를 의존하지 않는다.
- 모든 콘텐츠에 출처, 외부 ID, 최초 발견일, 마지막 확인일, 수집 방식을 기록한다.
- 동일 광고가 여러 고객에게 발견되어도 공용 원본은 한 번만 수집·분석하고, 고객별 관찰 관계와 인사이트는 분리한다.
- 삭제·비공개·만료된 콘텐츠와 미디어 사용 권한을 고려해 원본 전체를 불필요하게 영구 저장하지 않는다.
- 수동 등록은 자동 수집의 대체 흐름이 아니라 누락 콘텐츠를 보완하는 fallback으로 제공한다.
...
등록 후에는 시스템이 경쟁사별 신규 광고와 관련 콘텐츠를 주기적으로 탐색한다.
